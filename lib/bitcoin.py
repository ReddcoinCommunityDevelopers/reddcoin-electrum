...
def hash_160_to_bc_address(h160, addrtype=61):
...
def SecretToASecret(secret, compressed=False, addrtype=61):
    vchIn = chr((addrtype+128) & 255) + secret
    if compressed: vchIn += '\01'
    return EncodeBase58Check(vchIn)


def ASecretToSecret(key, addrtype=61):
...
def msg_magic(message):
    ...
    return "\x19Reddcoin Signed Message:\n" + encoded_varint + message
...
class EC_KEY(object):
    ...
    def sign_message(self, message, compressed, address):
        ...
        signature = private_key.sign_digest_deterministic(Hash(msg_magic(message)), hashfunc=hashlib.sha256,
