...
from reddcoin_electrum.util import print_error
from reddcoin_electrum.bitcoin import is_valid
from reddcoin_electrum import WalletStorage, Wallet

Gdk.threads_init()
APP_NAME = "Reddcoin Electrum"
import platform
MONOSPACE_FONT = 'Lucida Console' if platform.system() == 'Windows' else 'monospace'

from reddcoin_electrum.util import format_satoshis, parse_URI
from reddcoin_electrum.network import DEFAULT_SERVERS
from reddcoin_electrum.bitcoin import MIN_RELAY_TX_FEE
...
def run_network_dialog( network, parent ):
    ...
    add_help_button(host_box, 'The name, port number and protocol of your Reddcoin Electrum server, separated by a colon. Example: "ecdsa.org:50002:s". Some servers allow you to connect through http (port 80) or https (port 443)')
    host_box.show()
...
class ElectrumWindow:
    ...
    def __init__(self, wallet, config, network):
        ...
        title = 'Reddcoin Electrum ' + self.wallet.electrum_version + '  -  ' + self.config.path
    ...
    def do_send(self, w, data):
        ...
        if not is_valid(to_address):
            self.show_message( "invalid reddcoin address:\n"+to_address)
            return
