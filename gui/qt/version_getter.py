...
class UpdateLabel(QLabel):
    ...
    def mouseReleaseEvent(self, event):
        dialog = QDialog(self)
        dialog.setWindowTitle(_('Reddcoin Electrum update'))
        dialog.setModal(1)

        main_layout = QGridLayout()
        main_layout.addWidget(QLabel(_("A new version of Reddcoin Electrum is available:")+" " + self.latest_version), 0,0,1,3)
